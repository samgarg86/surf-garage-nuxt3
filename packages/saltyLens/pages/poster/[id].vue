<template>
  ...
  <div
    class="grid md:grid-cols-[55%_auto] gap-1 md:gap-6 mb-2 max-w-screen-container mx-auto"
  >
      ...
      <div class="md:mt-3 mb-1">
        ...
        <ul v-if="tags.page?.length" class="tags mb-1 text-right md:text-left">
          <template v-for="tag in tags.page" :key="tag">
            <li
              v-if="tag !== 'Home'"
              class="inline-block text-sm px-1 leading-9 mr-1 last:mr-0 bg-grey-10"
            >
      ...
      <Accordion class="mt-3 md:mt-5">
        ...
        >
          <p class="text-sm text-justify">
            {{ $t('art.accordion.materials.descPosters') }}
          </p>
        </AccordionItem>
        <AccordionItem
