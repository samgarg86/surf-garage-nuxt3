...
export default defineNuxtPlugin(async () => {
  ...
  if (!isCacheValid) {
    console.log(
      'Fetching site nav and tags from Contentful (should only happen once per day)'
    )
    ...
    if (process.env.CONTENTFUL_LOGGING_ENABLED === 'true') {
      ...
      console.log(
        `[${time}] CONTENTFUL API: getEntries - content_type: hamburgerMenu - source: siteNav.plugin`
      )
      console.log(`[${time}] CONTENTFUL API: getTags - source: siteNav.plugin`)
    }

    const { items } = await client.getEntries({
      content_type: 'hamburgerMenu',
      include: 10
    })
