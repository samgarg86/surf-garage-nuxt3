<template>
  <header class="fixed z-10 left-0 top-0 w-full text-center py-1.5 bg-black">
    ...
    <div class="flex absolute right-1 top-2 md:top-1 md:right-2 md:items-start">
      <LazyLanguageSwitcher :showLanguageName="false" @click="toggleLangExpanded" theme="dark" mode="dropdown" :class="['header-lang', { 'isExpanded': langExpanded } ]"/>
...
<style>
.header-lang {
  @apply relative overflow-hidden w-6 h-4.5 text-1.8 mobile:hidden;
  ...
  .language-switcher {
    ...
    a {
      @apply pl-1.5 pr-2.5 py-1 leading-tight;
    }
  }
