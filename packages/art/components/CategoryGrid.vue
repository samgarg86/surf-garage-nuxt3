<template>
  ...
  <div
    class="grid grid-cols-2 md:grid-cols-3 gap-1 md:gap-4 py-1 md:py-4 max-w-screen-container mx-auto"
  >
    <nuxt-link
    ...
    >
      <div class="absolute w-full left-0 bottom-0 p-1 md:p-4">
        <p
          class="art-category-grid-item__title text-sm md:text-base font-primary font-extrabold md:mb-1"
        >
          {{ title }}
        </p>
        <p
          v-if="description"
          class="text-sm font-medium mobile:hidden leading-10 min-h-[7.5rem]"
        >
          {{ description }}
        </p>
      </div>
