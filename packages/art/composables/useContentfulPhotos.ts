export const useContentfulPhotos = () => {
  ...
  const fetchArtGalleryPage = async (slug, limit = 12) => {
    ...
    if (entries.value.items?.length) {
      const {
        fields: { title, description, shortDescription, bannerImage },
        metadata: { tags }
      } = entries.value.items[0];

      pageTags.value = tags.map((tag) => tag.sys.id).join(',');

      pageImages.value = pageTags.value
        ? await fetchImagesByTags(pageTags.value, limit)
        : [];
      pageTitle.value = title;
      pageDescription.value = description;
      pageShortDescription.value = shortDescription;
      pageMainImage.value =
        bannerImage?.fields?.file?.url || pageImages.value?.[0]?.url || '';
    }
    return entries;
  };
  ...
  const fetchHomepage = async () => {
    ...
    if (!entries.value?.items) {
      throw createError(
        "Uhh you caught us at a wrong time. We're out either surfing or shooting. Please check back in a few minutes"
      );
    }
