<template>
  <div
    class="py-5 md:py-10 bg-grey-30 px-1 md:px-2 blog-banner"
    :class="{ 'text-white bg-image': !!image }"
    :style="{ '--bg-img': `url(${image})` }"
  >
    <div class="max-w-[70rem] mx-auto">
      <h1 class="text-4xl md:text-[6rem] text-center mx-auto leading-none">
        {{ title }}
      </h1>
      <p
        v-if="description"
        class="font-avenir text-center text-base mt-2 leading-relaxed"
      >
        {{ description }}
      </p>
    </div>
...
const image = computed(() =>
  bannerImage?.fields.file.url
    ? `${bannerImage?.fields.file.url}?w=1000&fm=webp`
    : ''
);
