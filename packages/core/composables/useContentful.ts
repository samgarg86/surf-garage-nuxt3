  ...
  return {
    ...
    getEntries: async(params) => {
      ...
      const { data, error: responseError } = await useAsyncData(
          `getEntries-${uniqueId || ''}`,
          () => {
            logContentfulCall('getEntries', `content_type: ${rest.content_type || 'none'}`)
            return client.getEntries({...rest, include: params.include || 10, order: params.order || '-sys.updatedAt', locale: locale.value})
          }
      )
    ...
    getAssets: async(params) => {
      ...
      const { data, error: responseError } = await useAsyncData(
          `getAssets-${uniqueId || ''}`,
          async () => {
            logContentfulCall('getAssets', `tags: ${rest['metadata.tags.sys.id[in]'] || 'none'}, limit: ${rest.limit || 'default'}`)
            return await client.getAssets({...rest, order: rest.order || '-sys.createdAt', locale: locale.value})
          }
      )
      if (responseError.value) console.error(responseError.value)
      return data
    ...
    getFirstEntryOfType: async (content_type, loc) => {
      ...
      const { data, error: responseError } = await useAsyncData(
          ...
          () => {
            logContentfulCall('getFirstEntryOfType', `content_type: ${content_type}`)
            return client.getEntries({content_type, include: 10, locale: loc || locale.value})
          }
      )
      if (responseError.value) console.error(responseError.value)
